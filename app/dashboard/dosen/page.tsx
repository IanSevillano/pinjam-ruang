...
export default function AdminDashboard() {
  ...
  //   window.location.href = "/dashboard/dosen/peminjamanSaya";
  // };
    const handleDateClick = (dateStr: string) => {
  window.location.href = `/dashboard/dosen/peminjamanSaya?date=${encodeURIComponent(dateStr)}`;
};
    ...
    <div className="flex min-h-screen">
      ...
      <main className={`flex-1 p-4 transition-all duration-300 ${sidebarCollapsed ? 'md:ml-0' : 'md:ml-64'} mt-16 md:mt-0`}>
        <div className="calendar-container w-full overflow-x-auto">
          <h1 className="calendar-title">Dashboard Dosen</h1>
          <Calendar
            ref={calendarRef}
            events={events}
